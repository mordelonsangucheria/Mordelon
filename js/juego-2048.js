...
let g2Over = false;
let g2Running = false;
Object.defineProperty(window, 'g2048Running', { get: () => g2Running });
...
function g2Render(){
  const bd=document.getElementById('g2048Board');
  if(!bd || bd.style.display==='none') return; // Validación de visibilidad
  bd.style.display='grid';
  bd.style.gridTemplateColumns='repeat(4, 1fr)';
  bd.style.gap='8px';
  bd.innerHTML='';
  for(let r=0;r<4;r++){
    for(let c=0;c<4;c++){
...
(()=>{
  ...
  bd.addEventListener('touchend',e=>{const dx=e.changedTouches[0].clientX-sx,dy=e.changedTouches[0].clientY-sy;if(Math.abs(dx)>Math.abs(dy)){window.g2048Move(dx>0?'right':'left');}else{window.g2048Move(dy>0?'down':'up');}},{passive:true});
})();